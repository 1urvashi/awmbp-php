        ...
        <div class="col-md-8 col-md-offset-2">
            <div class="box box-primary">
                <div class="box-header with-border">
                    <h3 class="box-title">@if(!isset($auction))Create @else Resubmit @endif Auction for - <b>@if(isset($auction)){{$auction->objectList->name}} ({{$auction->objectList->code}})@else{{$object->name}} ({{$object->code}})@endif</b></h3>
                    <h4 style="text-align:right;"><a  href="{{url('dealer/object/detail/'. $object->id )}}" class="btn btn-xs btn-success"><i class="fa fa-pencil-square-o"></i> View Vehicle</a></h4>
                </div>
                <!-- /.box-header -->
                <!-- form start -->
                @include('dealer.includes.status-msg')
                <form role="form" enctype="multipart/form-data" method="post" action="@if(isset($auction)){{url('dealer/auctions/'.$auction->id)}}@else{{url('dealer/auctions')}}@endif" action="{{url('dealer/auction/create')}}">
                    @if(isset($auction)) <input name="_method" type="hidden" value="PUT"> @endif
                    {{ csrf_field() }}
                    <div class="box-body">
                        @if(!isset($auction)) <input type="hidden" class="form-control" name="object_id" value="@if(isset($auction)){{$auction->objectList->id}}@else{{$object->id}}@endif"> @endif
                        <div class="form-group">
                            <label>Auction Title <span class="req">*</span></label>
                            @if(!empty($parentAuction->title))
                            <input type="text" class="form-control" id="title" name="title"
                            value="@if(isset($parentAuction)){{$parentAuction->title}}@else{{old('title')}}@endif">
                            @else
                            <input type="text" class="form-control" id="title" name="title"
                            value="@if(isset($auction)){{$auction->title}}@else{{old('title')}}@endif">
                            @endif
                        </div>
                        <div class="form-group">
                            <label>Start Price <span class="req">*</span></label>
                            <input type="text" class="form-control" id="start_price" name="base_price" value="@if(isset($auction)){{$auction->base_price}}@else{{old('base_price')}}@endif" onkeypress="return isFieldNumber(event)">
                        </div>
                        <div class="form-group">
                            <label>Minimum Increment <span class="req">*</span></label>
                            <input type="text" class="form-control" name="min_increment" value="@if(isset($auction)){{$auction->min_increment}}@else{{old('min_increment')}}@endif" onkeypress="return isFieldNumber(event)">
                        </div>
                       <div class="form-group">
                           <label>Auction Types <span class="req">*</span></label>
                            <select name="type" class="form-control select2" id="auctionType">
                                @foreach($types as $key=>$type)
                                <option @if(isset($auction) && $auction->type == $key) selected @elseif(old('type') == $key) selected @endif  value="{{$key}}">{{$type}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group" id="buy_section" style="display:none;">
                            ...
                            <input type="text" class="form-control" id="buy_price" name="buy_price" value="@if(isset($auction)){{$auction->buy_price}}@else{{old('buy_price')}}@endif" onkeypress="return isFieldNumber(event)">
                        </div>
                        ...
                        <input type="hidden" name="object_id" value="{{ app('request')->input('id') }}"/>
                        @endif
