<div class="row">
    <div class="col-md-12">
        <div class="row">
             <div class="col-md-3">
                  <h3 class="box-title" style="font-weight: bold;">VEHICLE DETAILS</h3>
           ...
           @php($customersDetailAuction = !empty($user) && $user && $user->isAllowed('customersDetail') ? 1 : 0)
         ...
         @endif
         <form role="form" id="objectEdit" enctype="multipart/form-data" method="post" action="{{url('dealer/object/edit/'.$object->id)}}">
            {{ csrf_field() }}
            <div class="col-md-12 oreder">
                ...
                <div class="col-xs-6 item">
                    <div class="form-group">
                        ...
                        <input type="file" multiple class="form-control" data-max-size="32154"  id="images" name="images[]" accept="image/jpeg,image/gif,image/png,image/jpg">
                    </div>
...
min-height: 170px;" src="{{$image->image}}"
                             data-darkbox="{{$image->image}}" alt="Vehicle Image">
                        <a onclick="return confirm('Are you sure you want to delete this image?');" style="margin-top: 10px;" href="{{url('dealer/remove-watch-data/images/'.$image->id)}}" class="btn btn-danger">Remove</a>
                    </li>
                    @endforeach
                </ul>
            ...
            <div class="col-md-12 document">
                ...
                <ul class="users-list clearfix" id="sortable-docu">
                    ...
                    <li class="ui-state-default" data-id="{{ $attachment->id }}">   
                            ...
                            <a onclick="return confirm('Are you sure you want to delete this PDF?');" style="margin-top: 10px;" href="{{url('dealer/remove-watch-data/attachment/'.$attachment->id)}}" class="btn btn-danger">Remove</a>

                            @else --}}
                            <img style="min-width: 250px; min-height: 170px;" 
                            src="{{$attachment->attachment}}" data-darkbox="{{$attachment->attachment}}" alt="Vehicle Image">
                            <a onclick="return confirm('Are you sure you want to delete this attachment?');" style="margin-top: 10px;" href="{{url('dealer/remove-watch-data/attachment/'.$attachment->id)}}" class="btn btn-danger">Remove</a>
                        ...
                        <div class="tab-pane">
                            <div class="col-md-4 item">
                              <div class="row">
                                  ...
                                  @elseif($attrvalue->attribute->input_type == $attrvalue->attribute->getInputType(11))
                                  <div class="form-group">
                                      <label>{{$attrvalue->attribute->name}}<span class="req">*</span></label>
                                      <input type="text" class="form-control" id="yearPicker" name="attributeValue[{{$attrvalue->attribute->id}}]" value='{{$attrvalue->attribute_value}}' required/>
                                  </div>
                                  ...
                                  <div class="form-group">
                                   ...
                                   @elseif(($attrvalue->attribute->input_type == $attrvalue->attribute->getInputType(1)) )
                                   <div class="form-group">
                                       <label >{{$attrvalue->attribute->name}}<span class="req"></span></label>
                                       ...
                                       <input type="radio"  name="attributeValue[{{$attrvalue->attribute->id}}]"@if($value->attribute_value == $attrvalue->attribute_value) checked @endif value='{{$value->attribute_value.','.$value->color}}' />{{$value->attribute_value}} 
                                        </label>
                                       @endforeach
...
$('#make').on('change', function() {
       ...
       if(makeId) {
            $.ajax({
                url: '{{url("dealer/get/models")}}/'+makeId,
                type: "GET",
                dataType: "html",
