    ...
    <div class="log-in-wrap__wrap">
      <div class="log-in-wrap__log-in">
        <div class="log-in-wrap__start">
          <img src="{{URL::asset('css/frontend/assets/img/logo-black.svg')}}" alt="wmb logo" />
        </div>
        <div class="log-in-wrap__end log-in">
          <span class="title">{{trans('frontend.reset_txt')}}</span>
       
          <form action="{{ url('trader/password/email') }}" method="post" class="form-log-in">
            {!! csrf_field() !!}

            <input id="q_email" type="email" class="form-log-in__input" required placeholder="Enter Your Email" name="email" value="{{ old('email') }}">
